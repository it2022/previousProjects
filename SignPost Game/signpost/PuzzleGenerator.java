    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place squareOne : sq.successors()) {
                    Sq squareTwo = model.get(squareOne);
                    if (sq.sequenceNum() != 0 && squareTwo.sequenceNum() != 0
                            && sq.connectable(squareTwo)) {
                        nFound = 1;
                        found = squareTwo;
                        break;
                    } else if (sq.connectable(squareTwo)) {
                        nFound++;
                        found = squareTwo;
                    }
                }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place squareOne : sq.predecessors()) {
                    Sq squareTwo = model.get(squareOne);
                    if (sq.sequenceNum() != 0
                            && squareTwo.sequenceNum() != 0
                            && squareTwo.connectable(sq)) {
                        nFound = 1;
                        found = squareTwo;
                        break;
                    } else if (squareTwo.connectable(sq)) {
                        nFound++;
                        found = squareTwo;
                    }
                }
