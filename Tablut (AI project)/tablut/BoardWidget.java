...
class BoardWidget extends Pad {
     ...
     * Squares on each side of the board.
     */
    private Stack<Square> states = new Stack<>();
    /**
     * Squares on each side of the board.
     */
    private Stack<Square> states1 = new Stack<>();
    /**
    ...
    static final Color
            ...
            KING_COLOR = new Color(255, 110, 110),
    ...
    static final int
            SQUARE_SIDE = 30,
            BOARD_SIDE = SQUARE_SIDE * SIZE + OFFSET + MARGIN;
    ...
    private void drawPiece(Graphics2D g, Square s) {
        ...
        if (_board.get(s).side() != EMPTY) {
            int x = cx(s.col()) + padding / 2;
            int y = cy(s.row()) + padding / 2;
            g.setColor(_board.get(s).side()
                    == BLACK ? Color.black : Color.WHITE);
            g.fillOval(x, y, SQUARE_SIDE - padding, SQUARE_SIDE - padding);
            if (s == _board.kingPosition()) {
                g.setColor(KING_COLOR);
                g.fillOval(x, y, SQUARE_SIDE - padding, SQUARE_SIDE - padding);
            }
        }
    }
    ...
    private void click(Square s) {
        ...
        if (clicks == 1 && states1.empty()) {
            states.add(s);
            clicks += 1;
        }
        if (!states.empty() && !states1.empty()) {
            Move temp = Move.mv(states.pop(), states1.pop());
            _commands.offer(temp.toString());
        }
        repaint();
    }
